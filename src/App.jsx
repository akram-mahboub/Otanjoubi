...
/* ─── Static random seeds (no re-render jitter) ──────────────────────────── */
  ...
  return (
    ...
    >
      <svg width="90" height="160" viewBox="0 0 90 160" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* petals - 6 long curved petals radiating out */}
        {[0, 60, 120, 180, 240, 300].map((angle, i) => {
          ...
          return (
            <path
              key={i}
              d={`M45,80 C${cx1},${cy1} ${cx2},${cy2} ${ex},${ey}`}
              stroke="#e8e8f8"
              strokeWidth="1.8"
              fill="none"
              strokeLinecap="round"
...
/* ─── Character bubble ────────────────────────────────────────────────────── */
function CharacterBubble({ char, onClick }) {
        ...
        >
          ...
          {char.src ? (
            <img
              src={char.src}
              alt={char.name}
              style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "top center", filter: "contrast(1.1) brightness(0.95)" }}
            />
          ) : (
            <span style={{ fontSize: char.size * 0.38, zIndex: 2, position: "relative" }}>{char.placeholder}</span>
          )}
...
/* ─── Popup ───────────────────────────────────────────────────────────────── */
function CharacterPopup({ char, onClose }) {
  ...
  return (
    ...
    >
      {/* Diagonal lines bg - manga cross-hatching feel */}
      <div
        style={{
          position: "absolute",
          inset: 0,
      ...
      <div
        ...
        style={{
          ...
          width: "100%",
          background: "#04040c",
          border: `1px solid ${char.accentColor}66`,
          position: "relative",
      ...
      >
        ...
        >
          <div>
            <div style={{ fontFamily: "'Rajdhani','Quicksand',sans-serif", fontWeight: 700, fontSize: "13px", letterSpacing: "0.2em", color: "#000", textTransform: "uppercase" }}>
              {char.series}
            </div>
          </div>
          <button
        ...
        {/* Body */}
        <div style={{ padding: "32px 32px 28px", display: "flex", gap: "24px", alignItems: "flex-start" }}>
          {/* Avatar panel */}
          <div
            style={{
              flexShrink: 0,
          ...
          >
            ...
            {char.src ? (
              ...
              <span style={{ fontSize: "52px", position: "relative", zIndex: 2 }}>{char.placeholder}</span>
            )}
          </div>
        ...
        {/* Divider */}
        <div style={{ height: "1px", background: `linear-gradient(90deg, ${char.accentColor}44, transparent)`, margin: "0 32px" }} />
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#010108", position: "relative", overflow: "hidden" }}>
      ...
      {/* ── Characters ── */}
      {CHARACTERS.map((c) => (
        <CharacterBubble key={c.id} char={c} onClick={setActiveChar} />
      ))}
      ...
      >
        ...
        >
          <h1 className="title-main" style={{ fontSize: "clamp(52px, 11vw, 120px)", lineHeight: 1 }}>
            Happy
          </h1>
        </div>
        <div
          ...
          {["2", "5"].map((d, i) => (
            ...
            >
              ...
              {[{top:0,left:0},{top:0,right:0},{bottom:0,left:0},{bottom:0,right:0}].map((p,j)=>(
                <div key={j} style={{position:"absolute",...p,width:"8px",height:"8px",borderTop: p.top===0?"1px solid #fff5":undefined,borderBottom:p.bottom===0?"1px solid #fff5":undefined,borderLeft:p.left===0?"1px solid #fff5":undefined,borderRight:p.right===0?"1px solid #fff5":undefined}} />
              ))}
              {d}
            </div>
          ))}
        ...
        >
          — click on the characters around you —
        </div>
      </div>

      {/* ── Popup ── */}
      {activeChar && <CharacterPopup char={activeChar} onClose={() => setActiveChar(null)} />}
    </div>
  );
}